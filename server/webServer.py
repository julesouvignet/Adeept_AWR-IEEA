...
if __name__ == '__main__':
    ...
    import RPi.GPIO as GPIO  # Assurez-vous que l'import est présent
    GPIO.setwarnings(False)
    GPIO.setmode(GPIO.BCM)   # <--- AJOUTEZ CETTE LIGNE ICI
    ...
    # >>> DEBUT MODIF ERREUR LOOP
    # 1. On prépare la connexion et la boucle (une seule fois)
    try:
        # On crée une nouvelle boucle proprement
    ...
    try:
        # 2. On configure le serveur sur le port 8888
        print('Starting WebSocket server on port 8888...')
        start_server = websockets.serve(main_logic, '0.0.0.0', 8888)
        
        # 3. On enregistre le serveur dans la boucle
        loop.run_until_complete(start_server)
        
        # 4. On allume les LEDs en vert pour confirmer le succès
        RL.pause()
        RL.setColor(0, 255, 64)
        print('Waiting for connection...')

        # 5. ON LANCE LA BOUCLE INFINIE (C'est cette ligne qui corrige l'erreur)
        loop.run_forever()

    except Exception as e:
        # En cas d'erreur, on affiche le détail et on éteint tout
...
#            import asyncio
#            asyncio.set_event_loop(asyncio.new_event_loop()) # Jules ajout pb  "no running event loop"
#            start_server = websockets.serve(main_logic, '0.0.0.0', 8888)
